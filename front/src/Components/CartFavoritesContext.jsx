...
export const CartFavoritesProvider = ({ children }) => {
  ...
  const fetchCart = async () => {
    try {
      const token = localStorage.getItem('token');
      const userId = localStorage.getItem('userId');
      const response = await axios.get(`https://video-game-store-aczz.onrender.com/users/${userId}/Cart`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const addToCart = async (gameId) => {
    ...
    try {
      ...
      await axios.post(`https://video-game-store-aczz.onrender.com/users/${userId}/Cart/${gameId}`, null, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const removeFromCart = async (gameId) => {
    try {
      const token = localStorage.getItem('token');
      const userId = localStorage.getItem('userId');
      await axios.delete(`https://video-game-store-aczz.onrender.com/users/${userId}/Cart/${gameId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchCart();
  ...
  const addToFavorites = async (gameId) => {
    try {
      const token = localStorage.getItem('token');
      const userId = localStorage.getItem('userId');
      await axios.post(`https://video-game-store-aczz.onrender.com/users/${userId}/favorites/${gameId}`, null, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchFavorites();
  ...
  const removeFromFavorites = async (gameId) => {
    try {
      const token = localStorage.getItem('token');
      const userId = localStorage.getItem('userId');
      await axios.delete(`https://video-game-store-aczz.onrender.com/users/${userId}/favorites/${gameId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      fetchFavorites();
